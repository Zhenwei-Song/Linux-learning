...
static void *auto_send_thread(void *argc)
{
    int ret;
    // printf("send_thread created\n");
    ...
    ret = connect(sockfd2, (struct sockaddr *)&server_addr, sizeof(server_addr));
    if (ret < 0) {
        perror("connect err");
        close(sockfd2);
        exit(EXIT_FAILURE);
    ...
    while (running) {
        ...
        strncpy(send_buf2, "auto sending...\n", sizeof(send_buf2)-1);
        sleep(5);
        ret = send(sockfd2, send_buf2, sizeof(send_buf2), 0);
        if (ret < 0) {
            perror("send err");
            close(sockfd2);
            exit(EXIT_FAILURE);
...
int main(int argc, char **argv)
{
    ...
    ret = pthread_create(&auto_send_tid, NULL, auto_send_thread, NULL);
    if (ret < 0) {
        perror("pthread_create err");
    ...
    pthread_join(auto_send_tid, NULL);
